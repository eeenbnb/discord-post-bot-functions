import getSubscriptions from "../lib/twich/getSubscriptions.mjs";

const getAllSubscriber = async (token) => {
  const allItem = [];
  const getItem = async (after = "") => {
    const item = await getSubscriptions(
...
export default getAllSubscriber;
